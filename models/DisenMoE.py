...
class MoEAdaptorLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        return multiple_outputs.sum(dim=-2), expert_outputs
...
class DisKoMoE(BaseModel):
    def __init__(self, args):
        super(DisKoMoE, self).__init__(args)
    ...
    def forward(self, batch_inputs):
        ...
        e_embed, disen_str = self.structure_moe(self.entity_embeddings(head))
        r_embed = self.relation_embeddings(relation)
        e_img_embed, disen_img = self.visual_moe(self.img_entity_embeddings(head))
        r_img_embed = self.img_relation_embeddings(relation)
        e_txt_embed, disen_txt = self.text_moe(self.txt_entity_embeddings(head))
        ...
        all_s, _ = self.structure_moe(self.entity_embeddings.weight)
        all_v, _ = self.visual_moe(self.img_entity_embeddings.weight)
        all_t, _ = self.text_moe(self.txt_entity_embeddings.weight)
